@extends('layouts.admin', ['page_title' => 'إضافة نمط URL - ' . $site->title])
...
<div class="pattern-form-container">
    <div class="pattern-form-card">
        ...
        <form method="POST" action="{{ route(request()->routeIs('admin.*') ? 'admin.analytics.patterns.store' : 'user.analytics.patterns.store', $site) }}">
            @csrf
...
<script>
    // If domain is a text input and user types, allow it
    document.addEventListener('DOMContentLoaded', function() {
        const domainSelect = document.getElementById('domain');
        if (domainSelect && domainSelect.tagName === 'SELECT') {
            domainSelect.addEventListener('change', function() {
                if (this.value === '' || this.value === 'other') {
                    // Allow custom domain input
                    const customInput = document.createElement('input');
                    customInput.type = 'text';
                    customInput.name = 'domain';
                    customInput.className = 'form-input-modern';
                    customInput.placeholder = 'example.com';
                    customInput.required = true;
                    customInput.style.marginTop = '8px';
                    if (!document.getElementById('custom-domain-input')) {
                        customInput.id = 'custom-domain-input';
                        domainSelect.parentElement.appendChild(customInput);
                    }
                } else {
